def inventory_report( inventory, gst=0.05, **filters):
    ...
    if "category" in filters:
        result= filter(lambda item:item[0]==filters["category"],inventory)
        
    if "max_price" in filters:
        result= filter(lambda item:item[3]==filters["max_price"],inventory)

    if "main_stock" in filters:
        result= filter(lambda item:item[2]==filters["main_stock"],inventory)
